...
export function DInvoiceModal({ onClose, onSuccess }: DInvoiceModalProps) {
  ...
  const calculateTotals = () => {
    ...
    return {
      ...
      isInterstate: customerState && customerState.toLowerCase() !== "andhra pradesh",
      gstBreakdown: form.watch("invoiceType") === "gst" 
        ? (customerState && customerState.toLowerCase() !== "andhra pradesh"
  ...
  const onSubmit = async (data: z.infer<typeof dInvoiceSchema>) => {
    try {
      ...
      const submitData = {
        ...formData,
        jobSheetId: parseInt(data.jobSheetId),
