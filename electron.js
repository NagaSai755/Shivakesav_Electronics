...
function startServer() {
  console.log('Starting SHIVAKESHAVA ELECTRONICS server...');
...
function createWindow() {
  console.log('Creating SHIVAKESHAVA ELECTRONICS window...');
  ...
  const loadingHTML = `
    <html>
      <head>
        <title>SHIVAKESHAVA ELECTRONICS</title>
        <style>
          body { 
      ...
      <body>
        <div class="logo">📱 SHIVAKESHAVA ELECTRONICS</div>
  ...
  const checkServer = () => {
    ...
    mainWindow.loadURL('http://localhost:5000').then(() => {
      console.log('SHIVAKESHAVA ELECTRONICS application loaded successfully!');
    }).catch((error) => {
        ...
        const errorHTML = `
          <html>
            <body style="font-family:Arial;text-align:center;padding:50px;background:#ffebee;">
              ...
              <h2>SHIVAKESHAVA ELECTRONICS</h2>
...
app.whenReady().then(() => {
  console.log('Electron is ready - starting SHIVAKESHAVA ELECTRONICS...');
  createWindow();
});
